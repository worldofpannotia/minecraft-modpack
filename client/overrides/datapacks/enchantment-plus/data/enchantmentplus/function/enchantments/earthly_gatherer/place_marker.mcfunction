...
# Place Marker
summon minecraft:marker ~ ~ ~ {Tags:["eplus.block_marker","eplus.eg_marker"]}
data modify entity @n[type=minecraft:marker,tag=eplus.eg_marker] data.UserUUID set from entity @s UUID
...
data modify entity @n[type=minecraft:marker,tag=eplus.eg_marker] data.EarthlyGathererBlockId set from entity @n[type=minecraft:item_display,tag=eplus.id_marker] item.id
kill @n[type=minecraft:item_display,tag=eplus.id_marker]

# Schedule
function enchantmentplus:enchantments/earthly_gatherer/schedule
