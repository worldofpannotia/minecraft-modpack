# Find Block
$execute positioned ~$(PosX) ~ ~$(PosZ) positioned ~-$(Level) ~ ~-$(Level) if block ~ ~ ~ #enchantmentplus:extended/is_pathable if predicate enchantmentplus:break_safety run function enchantmentplus:enchantments/extended/pathing/convert
...
# Continue
execute if score %PosX eplus.temp matches 0.. if score %PosZ eplus.temp matches 0.. if predicate enchantmentplus:break_safety run function enchantmentplus:enchantments/extended/pathing/axis_y with storage enchantmentplus:data Macros.Extended{}
