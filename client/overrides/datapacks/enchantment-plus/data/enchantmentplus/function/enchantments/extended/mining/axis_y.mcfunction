# Find Block
$execute positioned ~$(PosX) ~ ~$(PosZ) if predicate enchantmentplus:check_tool if predicate enchantmentplus:break_safety run function enchantmentplus:enchantments/extended/mining/break
...
# Continue
execute if score %PosX eplus.temp matches 0.. if score %PosZ eplus.temp matches 0.. if predicate enchantmentplus:break_safety run function enchantmentplus:enchantments/extended/mining/axis_y with storage enchantmentplus:data Macros.Extended{}
