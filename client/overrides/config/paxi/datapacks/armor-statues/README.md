...
- Added 'Item Frame animation time': Sets the amount of time that item frames can be detected and animate armor
  stands. (16 is recommended) Defaults to 0.
- Saved books now label if they have a saved pose in their Lore. You can name a saved pose by first naming the Armor
  Stand.

- Added a new 'states' system to Armor stand books. These can be found on the first page and include Repeat, Undo, and
  Redo.

1. Repeat - allows you to perform the same action multiple times.
2. Undo/Redo - allows you to revert multiple actions. The number of times that actions can be undone is based on the Max
   Undo States. This resets every time a different armor stand is edited.


- Added Item Frame Animators.

1. Disabled by default.
2. Allows players to place books(with a saved pose) into item frames to animate armor stands. You can animate them by
   placing redstone dust in the space with the item frame, then powering it.
